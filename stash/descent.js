...
function* descend(items, key, get_value, get_next) {
  ...
  if (entries.length > 1)
    for (const [k, group] of entries) {
      const values = group.map(get_value);
      const children = [...descend(values, key, get_value, get_next)];
      const ret = { key: k, values };
      if (children.length) Object.assign(ret, { children });
      yield ret;
    }
}
...
function* directory(options) {
  ...
  for (const { values, ...item } of descend(
    trimmed,
    first,
    second,
    split_seg
