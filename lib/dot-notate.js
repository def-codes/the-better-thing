    ...
    tx.iterator(
      tx.comp(
        ...
        tx.filter(
          node =>
            !(
              store.has([node, TYPE, n(EDGE)]) ||
              // HACK: varibles and literals are also blank nodes.  but we don't
              // capture that in the dot info.  This is an ad-hoc alternative to
              // information generally tying back Dot statements to the things
              // being represented.
              store.has([node, TERM_TYPE, VARIABLE]) ||
              store.has([node, TERM_TYPE, BLANK_NODE])
            )
        ),
        tx.mapcat(bn => [
