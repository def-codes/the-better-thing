...
const apply_rules = (rules, source_store, target_store) => {
  ...
  for (const { name, antecedent, consequent } of rules) {
    ...
    if (name === "CopyRule")
      console.log(`source_store.triples`, source_store.triples);

    const matches = sync_query(source_store, antecedent) || [];

    for (const record of matches) {
      if (name === "CopyRule") console.log(`record`, record);

      for (const { subgraph: template, ground } of template_parts) {
        if (name === "CopyRule")
          console.log(`template`, curied_triples(template));
        ...
        if (bound.every(is_ground_triple)) {
          if (name === "CopyRule") console.log(`FULLY GROUND`);
