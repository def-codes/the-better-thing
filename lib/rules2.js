...
// Assumes mapping always returns a value
const map_terms = (triples, mapping) =>
  triples.map(triple => triple.map(term => mapping(term)));
...
*/
const apply_rule = (rule, source_store, target_store) => {
  ...
  for (const record of matches) {
    for (const { subgraph: template, ground } of template_parts) {
      ...
      const master_mapping = term => {
        ...
        // record.
        return term.termType === "BlankNode" ? map_template_bnode(term) : term;
      };

      const bound = map_terms(template, master_mapping);
      ...
      if (bound.every(is_ground_triple)) intermediate.into(bound);
        ...
        const mapping = simple_entailment_mapping(target_store, incoming_store);
        if (mapping.size === 0) {
  ...
  target_store.add(intermediate.triples);
};
