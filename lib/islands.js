...
const islands_from = (input_triples, node_predicate) => {
  // create a transient store to index the triples
  const store = new RDFTripleStore(input_triples);

  // adapt store to graph
  const graph = rdf_triple_store_graph({ store });
  ...
  // how else can you think of this... this expands the graph by some degree, in this case 1
