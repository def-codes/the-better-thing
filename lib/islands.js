 ...
 */
const islands_from = (input_store, predicate) => {
  // collect subgraph comprising only the matching nodes
  const matching_triples = [...nodes_matching(input_store.triples, predicate)];
  ...
  const islands = components.map(ids => [
    ...nodes_matching(input_store.triples, t => ids.has(t)),
  ]);
