...
function* basic_node_facts(node, label) {
  yield [node, RDF_TYPE, NODE_TYPE];
  yield [node, LABEL, l(label || term_label(node))];
  // HACK: see note to dot-notate
  yield [node, TERM_TYPE, n(`def:${node.termType}`)];
}
...
exports.triples_to_dot_description = function*(store) {
  ...
  for (const [s, p, o] of store) {
    ...
    if (!is_node(o)) {
      target = b();
      yield* basic_node_facts(target, o.toString());
    }
    const edge = b();
