...
const { is_blank_node, bnodes_in } = require("./rdf-operations");
const {
  constructors,
  ascii_notate: notate,
  sat_prep,
...
const { describe_findings } = require("./simple-bnode-mapping-helpers");
...
// Tell the model which mappings are known to be impossible because they didn't
// match the target.  Some such pairings arise in the form of conditions stated
// on other nodes.  We can't eliminate them during the collection because it
// requires all findings to be in.  Instead we're just asserting to the SAT
// model that such mappings are known to be false.  It would perhaps be more
// efficient to eliminate them in a follow-up pass, since this would mean the
// removal of variables versus the addition of clauses.  But this was a simpler
// way to get the correct behavior.
...
function simple_bnode_mapping(A, B) {
  ...
  // Unit propagation would do that in a domain-agnostic way
...
module.exports = { simple_bnode_mapping };
