...
const considering = findings =>
  tx.reduce(
    tx.groupByMap({
      key: _ => _[0],
      group: [() => new Set(), x => x, (acc, [, val]) => acc.add(val)],
    }),
...
function* process_state({ A, B, mapping, looking_at: node }) {
  ...
  const query = [
    // You have to have an indexed interface for this to make sense...
    // ...clauses(B._store.indexS, ([, p, o]) => [v("ZZZ"), p, sub(o)]),
    // ...clauses(B._store.indexO, ([s, p]) => [sub(s), p, v("ZZZ")]),
    ...(function* () {
      for (const [s, p, o] of B.triples) {
        if (node === s) yield [v("ZZZ"), p, sub(o)];
        if (node === o) yield [sub(s), p, v("ZZZ")];
      }
    })(),
  ];
    ...
    findings[looking_at.value] = tx.reduce(
      tx.groupByMap({
        ...
        group: [
          () => [],
          x => x,
          (acc, val) => (acc.push(val.conditions), acc),
        ],
      }),
