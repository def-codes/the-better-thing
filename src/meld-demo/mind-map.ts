...
// 
// The hashtags are identifiers.  They can be identifiers of anything.  They can
// be identifiers of things that don't exist yet, but could exist.  In such a
// case, the *default* behavior (of scrolling to the element which has the
// hashtag as its id), is of no consequence.  Better would be to *create* the
// thing.  And it can be created with context, because there may be statements
// *about* X already.  Even if you assume all subjects in a graph are given a
// home (in the document), there's nothing stopping you from linking to such
// anchors.  In doing so, you can make a simple hashtag link a creative tool,
// since you can capture in that link a lot of context.  Patterns seen in that
// context serve as the basis of "graphical" polymorphism.  The tools to
// implement this are available and simple.  The challenge is devising a
// language that can exploit it.  I argue that RDF must be the substrate of any
// such language with any hope of survival.
// 
// So we
// - override a default behavior of the browser
//   - by interpreting a stream of hashtags
//     - that we presume is fed by the user's visitation
//       - or something authorized by the user
//
// - if the thing exists in the graph... or wait
//
// - assert the name of that hashtag as being linked by representations of these events.
// - 
// - reify the events (hashchange)
// - render something with anchor links to real or imaginable things in that model.
// - whether the thing "existed" or not, before, it will be represented.
// - then we can at least talk about it
//   - and listen to it.
//
//   - we presume intelligent things come with messages that we will at best
//     partially understand.
//
//   - listening, paying attention, trying to get the most out of those alien
//     heralds.  Be at rest.
//
...
//
//   "Facts" comes closest to the presumption of authority, but I think it's
//   fair to say that "facts" are undecidable by default these days.
//
//   I tend to use the word "claims" for this class of things.
//
// Values
//
//   Cohabitating the world with "claims of truth" are things called "values".
//
//   Values have no strings attached.  Truly.
//
// Claims and values
//
...
//
//    A claim is a value with a predicate.
//
//   How can we *tell* it's a claim?
//
//     Claims are also values, and they cannot change the semantics of what
//     values say is true about values.
//
...
//     represented.
//
//     How much say?  How must of that territory can you claim?  Well, you must
//     always be able to look at things at a lower level of abstraction.  In
//     which case other domains may have stronger precedence.  There must be a
//     way of weighting this, too.
//
//  Planar forcefield arbitration scheme
//
//     make a physical map of the domains, so that the domains exert some forces
//     over the placement of their representations.  Add bodies to the
//     forcefield representing values of arbitration properties.
//
//     and then you can have values of arbitration properties represented in a
//     plane.  In other words, you should be able to use arbitrary (and
//     including process-based)
// 
// Things-in-themslves
//
//     Semantics-driven transformations made at higher abstration layers must
//     not obscure the value-nature of the underlying supertypes.  How is that
//     possible?  By constraining the algebra of trait-exhibiting transforms to
//     a set of conflict-free operations.  That is, a family of terms where
//     contradiction is not possible.
//
//     Now, this technique makes no guaranteed invariant.  Even in the absence
//     of contradiction, we cannot for example, make a rule asserting that all
//     things shall be visible.  We *can* say, "For all Things, that Thing has a
//     Representation," and find words to express the proposal that the
//     representation should be ever faithful.  But invisible in that claim
//     would be its author's ignorance of the existence of a viewport, where
//     duly-represented things can become occluded for reasons quite alien to
//     the rule's upholders (including their representatives), which of force
//     know nothing of their outer context.
//
// Representation over reference
// 
//     When representing things whose terms come from "other" namespaces, you
//     must refer to them in some way.
// 
//     (Note that verifying the resource's _data_ does *not* entail verifying its
//     claims.)
//
//     This is a bit like value over identity.  Let the thing speak for itself,
//     let me not take it on authority.
// 
// Limited conflict resolution
//
//     So far, we've avoided introducing the possibility of contradiction into a
//     family of statements.  Contradiction, as you probably know, has drastic
//     consequences for the outlook of a language.
//
//     Prevention is the cure.  Prefer *open* semantics.
// 
// Optionality over requirement
// 
//     Despite our efforts, some Conflict is unavoidable.
//
//     Conflict may also be *unresolvable*, which may have consequences.
//
//     Goals with subprocesses mired in conflict may decide that the juice is
//     not worth the squeeze.  If the subprocess's goal was not a *required*
//     input (but rather a body in a composition.)

// Disjunction
//
//     Disjunction
//
//     Disjunctive statements require negative assertions: Alice and Bob are
//     different people; therefore, you can't be Bob if you're Alice, and vice
//     versa.
//
// Negation in RDF
//
//     RDF lacks a first-class notion of negation.  We cannot directly say,
//     "Alice is not Bob".  But we can imply as much by adopting ontological
//     terms, whose logical consequences are formalized as _entailment regimes_.
//
//  
//
//     Physical bodies are spatially *disjunctive*: they cannot occupy the same
...
//
//     Computational complexity has direct consequences for the capability of a
//     thing.
//
...
//
//
//     They do it naturally.  It is in their nature as solid bodies.  That
//     property comes from a much lower abstraction layer than our typical
//     discourse.  That is, we don't think it a special property of a thing; it
//     is accomplished through no power of its own, or at least no power
//     specially its own.  It is certainly not a distinguishing trait of any
//     kind of thing, a la: Cats are feline things that can't occupy the same
//     space as other things.
//
//     The universality of physical properties must be exploited as having
//     descriptive power in abstracted contexts.
//
// Conflict resolution (dup)
//
//     If the conflict arose in the course of a goal, that goal will remain
//     unattainable as long as it is blocked by an unresolvable conflict.
//
//     
// 
...
// 
//     But if an implementation is able to verify the integrity of a resource
//     bearing claims about that domain's manifestations, then it must give
//     priority to claims coming from the domain in question, all in matters of
//     dispute then defined.
//
//
// Polymorphism
//
//     Polymorphism is situational response.
//
//     Are *things* polymorphic?  Sort of.
//
//     Thing are polymorphic in that things are handled by other things in
//     different ways based on their characteristics.
//
//     While dispatch on arbitrary composite values is powerful, it is by itself
//     too situated to serve as the basis of traits that exhibit contextual
//     awareness.
//     
// Macro expansion
//
//     Indeed, there are few good reasons to favor one group of things over
//     another based on a description of their characteristics.
//
...
//    
// 
//  Claims can include values, and
//   values can refer to claims.  The difference is that truth cannot be imputed
//   to values *per se*, i.e. they have no truth value.  Values
//
//   They cannot be used as predicates.
//
//
//
// Representation
...
//
//   Once a thing is being represented, we enter the Pure Data world, the realm
//   of ideals, a 
//
